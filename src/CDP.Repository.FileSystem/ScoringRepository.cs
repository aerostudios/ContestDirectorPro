...
namespace CDP.Repository.FileSystem
{
    ...
    public sealed class ScoringRepository : RepositoryBase, IScoringRepository
    {
        ...
        public ScoringRepository(string filePath, ILogger logger) : base(filePath, logger) { }
        ...
        /// <summary>
        /// Deletes the pilotRoundScore for a given contest.
        /// </summary>
        ...
        public async Task<Result<Dictionary<int, IEnumerable<TimeSheet>>>> ReadAsync(string contestId)
        {
            if (string.IsNullOrEmpty(contestId))
            {
                return Error<Dictionary<int, IEnumerable<TimeSheet>>>(null, $"{nameof(contestId)} cannot be null or empty.");
            }
            ...
            {
                return Error<Dictionary<int, IEnumerable<TimeSheet>>>(null, new Exception($"A pilot with the id {contestId} could not be found"));
            }

            var returnObj = new Dictionary<int, IEnumerable<TimeSheet>>();

            foreach (var scoreGroup in scoresToReturn)
            {
                var pilotTimeSheets = new List<TimeSheet>();
                var roundOrdinal = scoreGroup.First().RoundOrdinal;

                if (!returnObj.ContainsKey(roundOrdinal))
                {
                    returnObj.Add(roundOrdinal, new List<TimeSheet>());
                }
                ...
                returnObj[roundOrdinal] = pilotTimeSheets;
            }
